...
function updateDashboard() {
  ...
  if (sideStreak) {
    sideStreak.textContent = state.streak || 0;
  }
  ...
  if (lesson && titleEl) {
    ...
    if (metaEl) {
      metaEl.textContent = lesson.level + ' · ' + (lesson.category || '');
    }
  } else if (titleEl) {
    titleEl.textContent = 'Alle Lektionen abgeschlossen!';

    if (metaEl) {
      metaEl.textContent = 'Herzlichen Glückwunsch';
    }
  }
...
function renderDictionary(filter) {
  ...
  items.forEach(w => {
    ...
    el.innerHTML =
      ...
      '<div class="pl">' + escapeHtml(w.pl) + '</div>' +
      (w.example
        ? '<div class="ex">' + escapeHtml(w.example) + '</div>'
        : '');
...
function updateStats() {
  ...
  grid.innerHTML =
    ...
    '<div class="stat-card">' +
      ...
      '<span class="stat-value">' + (state.streak || 0) + ' Tage</span>' +
    '</div>' +

    '<div class="stat-card">' +
      '<span class="stat-label">Wörter</span>' +
      '<span class="stat-value">' + (state.learnedWords || []).length + '</span>' +
    '</div>' +

    '<div class="stat-card">' +
      '<span class="stat-label">Level</span>' +
      '<span class="stat-value">' + getCurrentLevel(state) + '</span>' +
    '</div>';
}
...
function getAllLessons() {
  ...
  if (typeof LESSONS_01_10 !== 'undefined') {
    chunks.push(...LESSONS_01_10);
  }

  if (typeof LESSONS_11_25 !== 'undefined') {
    chunks.push(...LESSONS_11_25);
  }

  if (typeof LESSONS_26_50 !== 'undefined') {
    chunks.push(...LESSONS_26_50);
  }

  if (typeof LESSONS_51_75 !== 'undefined') {
    chunks.push(...LESSONS_51_75);
  }

  if (typeof LESSONS_76_100 !== 'undefined') {
    chunks.push(...LESSONS_76_100);
  }
